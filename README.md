# Pianki - Anki卡片制作工具

一个现代化的Anki卡片制作Web应用，支持Markdown格式、实时预览、图片插入，可导出为标准.apkg格式。

![Version](https://img.shields.io/badge/version-0.1.8-blue)
![License](https://img.shields.io/badge/license-MIT-green)
![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey)

## ✨ 核心特性

### 📋 Markdown编辑器（实时预览）
- **左右分栏设计**：左侧编辑，右侧实时预览
- 支持标准Markdown语法（粗体、斜体、标题、列表等）
- 工具栏快捷按钮（粗体、斜体、代码、图片）
- 支持表格、引用、代码块等高级格式
- 所见即所得的编辑体验

### 🖼️ 灵活的图片处理
- **方式1**：点击工具栏"📷 插入图片"按钮
- **方式2**：直接在编辑器中按 Ctrl+V (Mac: Cmd+V) 粘贴
- 图片自动插入到光标位置
- 使用相对路径，更简洁可移植
- 支持文字与图片混排
- 支持格式：JPG、PNG、GIF、WebP

### 💾 数据管理
- 多牌组支持
- 卡片创建、编辑、删除
- 实时更新卡片计数
- 轻量级 JSON 文件数据库
- 自动数据持久化
- .apkg 文件导入/导出

### 📦 Anki导出
- 导出为标准 .apkg 格式
- 完美支持Markdown转HTML
- 自动打包图片资源
- 兼容Android/Desktop Anki

### 🎨 现代化UI
- 美观的确认对话框（替代浏览器默认弹窗）
- 响应式设计
- 流畅的动画效果
- 简洁直观的界面

## 🚀 快速开始

### 安装依赖

```bash
npm install
```

### 启动开发服务器

```bash
# 同时启动前端和后端
npm run dev
```

访问 http://localhost:3000/ 开始使用！

### 其他命令

```bash
# 仅启动后端
npm run dev:backend

# 仅启动前端
npm run dev:frontend

# 构建生产版本
npm run build

# 启动生产服务器
npm start
```

## 🛠️ 技术栈

### 前端
- **框架**: React 18 + TypeScript
- **构建工具**: Vite
- **Markdown**: react-markdown + remark-gfm
- **HTTP客户端**: Axios

### 后端
- **运行时**: Node.js + Express + TypeScript
- **数据库**: 自定义 JSON 数据库（基于 fs/promises）
- **文件上传**: Multer
- **Markdown处理**: Marked
- **Anki 导入/导出**: anki-apkg-export + sql.js

### 桌面应用（Tauri）
- **框架**: Tauri 2.x
- **架构**: Sidecar 模式（Node.js 后端作为独立进程）
- **打包工具**: @yao-pkg/pkg
- **平台支持**: Windows, macOS, Linux

## 📖 使用说明

### 创建卡片

1. 选择或创建一个牌组
2. 点击"➕ 新建卡片"按钮
3. 在左右分栏的Markdown编辑器中编辑内容：
   - **左侧**：输入Markdown格式文本
   - **右侧**：实时预览渲染效果
4. 插入图片：
   - 点击工具栏"📷 插入图片"
   - 或直接粘贴剪贴板中的图片
5. 点击"保存"

### 导出到Anki

1. 选择要导出的牌组
2. 点击"📦 导出为 .apkg"按钮
3. 桌面版会弹出“保存文件”对话框，选择位置后自动写入；浏览器版将直接下载
4. 在 Anki 中导入该文件

### Markdown语法示例

```markdown
# 一级标题
## 二级标题

**粗体文字** *斜体文字* ~~删除线~~

- 列表项 1
- 列表项 2

> 引用文本

`代码片段`

![图片](/uploads/xxx.png)
```

## 📁 项目结构

```
pianki/
├── frontend/                 # 前端应用
│   ├── src/
│   │   ├── components/       # React组件
│   │   │   ├── MarkdownEditor.tsx    # Markdown编辑器
│   │   │   ├── CardEditor.tsx        # 卡片编辑器
│   │   │   ├── CardList.tsx          # 卡片列表
│   │   │   ├── DeckSelector.tsx      # 牌组选择器
│   │   │   └── ConfirmDialog.tsx     # 确认对话框
│   │   ├── App.tsx           # 主应用
│   │   ├── api.ts            # API调用
│   │   └── main.tsx          # 入口文件
│   └── package.json
│
├── backend/                  # 后端服务
│   ├── src/
│   │   ├── routes/           # API路由
│   │   │   ├── cards.ts      # 卡片路由
│   │   │   └── decks.ts      # 牌组路由
│   │   ├── database.ts       # 数据库初始化
│   │   ├── anki-export.ts    # Anki导出逻辑
│   │   ├── types.ts          # TypeScript类型
│   │   └── index.ts          # 服务器入口
│   ├── data/                 # 数据存储（自动生成）
│   │   └── pianki.db         # SQLite 数据库
│   ├── uploads/              # 上传的图片（自动生成）
│   ├── scripts/              # 数据迁移脚本
│   │   └── migrate-json-to-sqlite.ts
│   └── package.json
│
├── package.json              # 根配置文件
└── README.md                 # 项目文档
```

## 🔧 配置说明

### 服务器端口
- 开发环境（Web）：前端 http://localhost:3000，后端 http://localhost:3001
- 桌面版（Tauri）：后端端口自动选择，默认从 `9908` 开始，若冲突会在 `9908–9928` 之间递增尝试；前端在启动时接收 `backend-ready` 事件自动切换到实际端口

### 数据存储位置
- 数据库: `backend/data/pianki.db`
- 上传图片: `backend/uploads/`
- 旧版 JSON 备份（迁移时自动生成）: `backend/data/db.json.bak-*`

### 图片限制
- 最大文件大小: 5MB
- 支持格式: JPEG, PNG, GIF, WebP

## 💡 开发指南

### 添加新功能

1. **前端组件**: 在 `frontend/src/components/` 创建新组件
2. **API接口**: 在 `backend/src/routes/` 添加新路由
3. **数据模型**: 在 `backend/src/types.ts` 定义类型

### 数据库操作

后端数据已迁移到 SQLite，所有读写通过仓库函数完成，例如：

```typescript
import { createDeck } from './db/repositories/decks';
import { createCard } from './db/repositories/cards';

const deck = createDeck({ name: '示例牌组' });
createCard({
  deck_id: deck.id,
  front_text: 'Question',
  back_text: 'Answer'
});
```

如果仍保存着旧版 `db.json`，可以执行迁移脚本：

```bash
cd backend
npm run migrate:sqlite
```

脚本会自动备份 JSON 并写入 `backend/data/pianki.db`。

### Markdown到Anki的转换流程

1. 用户在编辑器中输入Markdown
2. 保存时使用相对路径（如 `/uploads/xxx.png`）
3. 导出时：
   - 提取Markdown中的图片链接
   - 读取图片文件并添加到.apkg包
   - 使用`marked`将Markdown转换为HTML
   - 生成Anki卡片

## 🐛 常见问题

**Q: 数据保存在哪里？**
A: 数据库文件位于 `backend/data/pianki.db`，图片保存在 `backend/uploads/` 目录。

**Q: 如何备份数据？**
A: 复制 `backend/data/pianki.db` 和 `backend/uploads/` 目录即可（若仍有 `.bak` 的 JSON 文件一并备份更安全）。

**Q: 导出的.apkg文件在Anki中无法打开？**
A: 确保：
- 牌组中至少有一张卡片
- 图片文件格式正确且未损坏
- Anki版本支持（建议2.1+）

**Q: 可以同时创建多个牌组吗？**
A: 可以！点击左侧边栏的"➕"按钮即可创建新牌组。

**Q: Markdown预览和Anki显示一致吗？**
A: 基本一致。应用会将Markdown准确转换为HTML，包括格式和图片。

## 📣 反馈与联系

- 邮箱：prehisle@gmail.com
- GitHub Issues：https://github.com/prehisle/pianki/issues/new/choose
- GitHub Discussions：https://github.com/prehisle/pianki/discussions
- QQ群：188193559（无法直接跳转时可手动搜索，或复制群号加入）

## 📄 许可证

MIT License

## 🙏 致谢

- [anki-apkg-export](https://github.com/repeat-space/anki-apkg-export) - Anki 包导出库
- [react-markdown](https://github.com/remarkjs/react-markdown) - React Markdown 渲染
- [marked](https://github.com/markedjs/marked) - Markdown 解析器
- [Tauri](https://tauri.app/) - 跨平台桌面应用框架
- [sql.js](https://github.com/sql-js/sql.js) - SQLite Wasm 实现（用于 .apkg 导入）

---

**开始制作您的Anki卡片吧！** 🎉

访问 http://localhost:3000/ 立即体验。
